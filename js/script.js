// Напишите нам
    ...
    link.addEventListener("click", function(event) {
      ...
      popup.classList.add("write-content-show");
    });
    close.addEventListener("click", function(event) {
      event.preventDefault();
      popup.classList.remove("write-content-show");
    });
...
//Товар добавлен
    ...
    addlink.addEventListener("click", function(event) {
      event.preventDefault();
      addpopup.classList.add("product-content-show");
    });
    addclose.addEventListener("click", function(event) {
      event.preventDefault();
      addpopup.classList.remove("product-content-show");
    });
    window.addEventListener("keydown", function(event) {
        if (event.keyCode === 27) {
